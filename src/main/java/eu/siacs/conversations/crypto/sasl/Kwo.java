...
public class Kwo extends SaslMechanism {
    ...
    public byte[] getClientFirstMessage(final SSLSocket sslSocket) {
        return account.getUsername().getBytes(Charset.defaultCharset());
    }
    
    @Override
    public byte[] getResponse(final byte[] challenge, final SSLSocket sslSocket) throws AuthenticationException {
        if(challenge == null || challenge.length == 0)
            return new byte[0];
        return getMessage(account.getPassword(), this.account.getContext(), new String(challenge));
    }

    private static byte[] getMessage(final String password, Context context, final String challenge) {
        String pw = createPassword(password, context, challenge);
        Log.d(LOGTAG, pw);
        return pw.getBytes(Charset.defaultCharset());
    }
